...
function handleAudioStream(response, onComplete) {
    ...
    function processChunk({ done, value }) {
        ...
        if (done) {
            if (pendingBase64Line.trim()) {
                try {
                    const binaryString = atob(pendingBase64Line.trim());
                    ...
                    console.error("Final base64 decode error:", e);
                }
            }
        ...
        pendingBase64Line += textChunk;
        const lines = pendingBase64Line.split("\n");
        pendingBase64Line = lines.pop() || "";
...
async function startInterview() {
    ...
    try {
        ...
        interviewSessionId = response.headers.get("X-Interview-Session");
...
async function submitAnswer() {
    ...
    try {
        const response = await fetch(submitAnswerApiUrl, {
            method: "POST",
            headers: interviewSessionId ? { "X-Interview-Session": interviewSessionId } : {},
            body: formData
        });
    ...
    try {
        const response = await fetch(getFeedbackApiUrl, {
            ...
            headers: {
                ...
                ...(interviewSessionId ? { "X-Interview-Session": interviewSessionId } : {})
            },
            body: JSON.stringify({})
        });
